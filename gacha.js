...
function pullGacha(coinType) {
    ...
    // Display the result
    document.getElementById('result').innerText = `You won: ${prize.name} (${prize.rarity}) - ${prize.stats}`;

    // Remove the prize from the pool
    removePrize(prize);
}

// Function to remove a prize from the pool
function removePrize(prize) {
    let pools = [commonItems, uncommonItems, rareItems];
    for (let pool of pools) {
        let index = pool.findIndex(p => p.name === prize.name);
        if (index !== -1) {
            pool.splice(index, 1);
            break;
        }
...
// Function to reset the prize pools
